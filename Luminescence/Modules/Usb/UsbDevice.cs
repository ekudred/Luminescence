...
public class UsbDevice : IDisposable
{
    ...
    public int ReadIntervalInMillisecs
    {
        get
        {
            lock (syncLock)
            {
                return readIntervalInMillisecs;
            }
        }
        set
        {
    ...
    private int DefaultInputReportLength = 64;
    ...
    private bool hasReportIds = true;
    ...
    public ushort VendorID_exp { get; } // добавил сам
    ...
    public void Write(byte[] userData)
    {
        ...
        lock (syncLock)
        {
            byte[] outputReport = new byte[userData.Length];
    ...
    public byte[] Read(int reportLength = -1)
    {
        lock (syncLock)
        {
            int length = reportLength;
            ...
            byte[] inputReport = new byte[length];
            int readBytes = ReadRaw(inputReport);
            byte[] data = new byte[readBytes];

            Array.Copy(inputReport, 0, data, 0, readBytes);
    ...
    private void ReadLoop()
    {
        ...
        while (asyncReadOn)
        {
            try
            {
                ...
                if (data.Length > 0 && InputReportArrivedEvent != null)
                {
                    InputReportArrivedEvent(this, new UsbReportEventArgs(data));
                }
            }
            catch (Exception)
            {
